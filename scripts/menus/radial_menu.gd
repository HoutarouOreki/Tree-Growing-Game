...
var center: Vector2 = Vector2(ProjectSettings.get_setting("display/window/size/viewport_width"), ProjectSettings.get_setting("display/window/size/viewport_height")) / 2:
	set(value):
		set_center(value)
...
# do not touch these
var slice_heights: Array[float] = [] # array of floats (0 to 1)
var slice_labels: Array[Label] = [] # array of labels
var slices: Array[String] = [] # array of strings
...
func _init(passed_slices: Array[String]):
	...
	title_label.custom_minimum_size = Vector2((menu_radius - menu_width) * 1.8, (menu_radius - menu_width) * 1.8)
	title_label.position = center - Vector2(0, 8) # adjust as necessary, needs to be brought up by 1/2 the font height to be centered
	...
	for i in range(num_items):
		...
		l.custom_minimum_size = Vector2(menu_width * 0.8, menu_width * 0.8)
		if (font != null):
			l.set("custom_fonts/font", font)
		var middle_angle: float = i * item_size + item_size / 2 + angle_offset
		l.position = center + Vector2(cos(middle_angle) * middle_radius, sin(middle_angle) * middle_radius)
		l.text = slices[i]
		add_child(l)
...
func _process(delta):
	...
	queue_redraw() # erases screen and calls _draw()
...
func _draw():
	for i in range(num_items):
		...
		slice_labels[i].position = center + Vector2(cos(middle_angle) * radius, sin(middle_angle) * radius) - slice_labels[i].size / 2
...
func set_center(new_center: Vector2):
	center = new_center
	title_label.position = center - Vector2(0, 8)
